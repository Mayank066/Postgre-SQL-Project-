-- APPLER SALES PROJECT - 1M ROWS DATASETS
...
-- Execution Time = 58.74ms
-- Planning Time = 0.19ms 
-- Execution time after indexing = 5-10ms 

EXPLAIN ANALYZE 
SELECT * FROM sales
WHERE product_id = 'P-58'

CREATE INDEX sales_product_id ON sales(product_id);
CREATE INDEX sales_product_id ON sales(product_id);
CREATE INDEX sales_sale_id ON sales(sale_date);


-- Exection time = 53.68ms
-- Planning time = 1.06ms
-- Execution time after indexing = 2-8ms
...
-- No claim
-- No sale 
-- Each must be laumched in 2 years 
...
(
SELECT 
store_name,year,LAG(total_sale, 1) OVER(PARTITION BY store_name ORDER BY year) as last_year_sale,
	total_sale as current_year_sale
FROM yearly_sales
)

SELECT 
store_name,year,last_year_sale,current_year_sale,
	ROUND(
...
SELECT tr.store_id,st.store_name,pr.paid_repaired,tr.total_repaired,ROUND(pr.paid_repaired::numeric/
...
(SELECT store_id,EXTRACT(YEAR FROM sale_date) as year,EXTRACT(MONTH FROM sale_date)
as month,SUM(p.price * s.quantity) as total_revenue
FROM sales as s
JOIN 
products as p
ON s.product_id = p.product_id
